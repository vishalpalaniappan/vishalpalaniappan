...
> [!NOTE]
> The trace can also be instrumented with semantic knowledge that describes the purpose that each logical piece serves in the systems design. This means that the design of the system itself can be reconstructed along with the purpose of each abstraction. When a failure occurs, in addition to describing what and how something happened, we can also describe why it happened in the context of the design. As a result, operational intelligence will be able to explain the failure at the design level and identify design flaws.
>
>  Looking at this a different way, we can also say that we know the purpose each abstraction serves, how it itends to do this and what it actually does (as backed by the real observations). So when a failure is identified, we understand how the purpose, intent and reality failed to align, allowing the design intelligence to ensure that these three components are aligned thorugh precisely motivated changes.
